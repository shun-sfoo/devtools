...
static int lept_parse_string(lept_context* c, lept_value* v) {
  ...
  char* s;
  size_t len;
  if ((ret = lept_parse_string_raw(c, &s, &len)) == LEPT_PARSE_OK) lept_set_string(v, s, len);
  return ret;
}
...
void lept_copy(lept_value* dst, const lept_value* src) {
  ...
  switch (src->type) {
    case LEPT_STRING: lept_set_string(dst, src->u.s.s, src->u.s.len); break;
    case LEPT_ARRAY:
      /* \todo */
      break;
    case LEPT_OBJECT:
...
int lept_is_equal(const lept_value* lhs, const lept_value* rhs) {
  ...
  switch (lhs->type) {
    case LEPT_STRING: return lhs->u.s.len == rhs->u.s.len && memcmp(lhs->u.s.s, rhs->u.s.s, lhs->u.s.len) == 0;
    case LEPT_NUMBER: return lhs->u.n == rhs->u.n;
    case LEPT_ARRAY:
